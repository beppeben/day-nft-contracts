...
describe("days_on_flow", ()=>{
  ...
  test("days_on_flow", async () => {    
    ...
    // add DOF series
    var args = [false, false, "desc", "img", "img2", "name", [0, 2], [wl_user, bob], 0.5, 2, 2.5, {}];
    ...
    expect(balance).toEqual("1000000015.49400000");
    ...
    // check bob's balance
    var [balance, error] = await getFlowBalance(bob);
    expect(balance).toEqual("40.00100000");
    ...
    // check bob's balance
    var [balance, error] = await getFlowBalance(bob);
    expect(balance).toEqual("39.50100000");

    // check contract balance
    var [balance, error] = await getFlowBalance(myself);
    expect(balance).toEqual("1000000015.99400000");
    ...
    var exp_res = {id: 2, seriesDescription: 'desc', seriesId: seriesId, seriesImage: 'img', seriesSmallImage: 'img2', seriesName: 'name', serial: 2, metadata: {}}
    ...
    var args = [true, true, "description", "img", "img2", "name2", [0, 2], [wl_user, bob], 0.5, 2, 2.5, {}];
    var [tx, error] = await sendTransaction("add_dof_series", [myself], args);
    expect(error).toBeNull();
